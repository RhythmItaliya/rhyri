...
const ClientPopup: React.FC<ClientPopupProps> = ({ onSelect, onClose }) => {
  ...
  const fetchClients = async () => {
    ...
    try {
      ...
      const fetchedClients: Client[] = querySnapshot.docs.map(
        (doc) =>
          ({
            ...doc.data(),
            id: doc.id,
  ...
  const updateClientsForCurrentPage = (clientsList: Client[]) => {
    const filteredClients = clientsList.filter((client) => {
      const searchString =
        `${client.clientName} ${client.clientEmail} ${client.clientTelephone || ""}`.toLowerCase();
  ...
  const hasNextPage =
    (pagination.pageIndex + 1) * pagination.pageSize < allClients.length;
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-gray-900 bg-opacity-50">
      <div
        className={`container max-w-full md:max-w-4xl bg-${isDarkTheme ? "black text-white" : "white text-black"} rounded-lg shadow-lg overflow-y-auto p-4 md:p-6`}
      >
                ...
                clients.map((client) => (
                  ...
                  >
                    <TableCell className="uppercase">
                      {highlightMatch(client.clientName, combinedFilter)}
                    </TableCell>
                    <TableCell>
                      {highlightMatch(
                    ...
                    <TableCell>
                      {highlightMatch(client.clientEmail, combinedFilter)}
                    </TableCell>
                  </TableRow>
                ))
              ) : (
...
const ClientSelectButton: React.FC<ClientSelectButtonProps> = ({
  onClientSelect,
}) => {
