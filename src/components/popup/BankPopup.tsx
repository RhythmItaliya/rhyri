...
const BankPopup: React.FC<BankPopupProps> = ({ onSelect, onClose }) => {
  ...
  const fetchBanks = async () => {
    ...
    try {
      ...
      const fetchedBanks: Bank[] = querySnapshot.docs.map(
        (doc) =>
          ({
            ...doc.data(),
            id: doc.id,
  ...
  const updateBanksForCurrentPage = (banksList: Bank[]) => {
    const filteredBanks = banksList.filter((bank) => {
      const searchString =
        `${bank.bankName} ${bank.bankAccountNumber} ${bank.bankBranchName || ""}`.toLowerCase();
  ...
  const hasNextPage =
    (pagination.pageIndex + 1) * pagination.pageSize < allBanks.length;
                ...
                banks.map((bank) => (
                  ...
                  >
                    <TableCell className="uppercase">
                      {highlightMatch(bank.bankName, combinedFilter)}
                    </TableCell>
                    <TableCell>
                      {highlightMatch(
