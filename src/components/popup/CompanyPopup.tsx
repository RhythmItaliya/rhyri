...
const CompanyPopup: React.FC<CompanyPopupProps> = ({ onSelect, onClose }) => {
  ...
  const fetchCompanies = async () => {
    ...
    try {
      ...
      const fetchedCompanies: Company[] = querySnapshot.docs.map(
        (doc) =>
          ({
            ...doc.data(),
            id: doc.id,
  ...
  const updateCompaniesForCurrentPage = (companiesList: Company[]) => {
    const filteredCompanies = companiesList.filter((client) => {
      const searchString =
        `${client.companyName} ${client.companyEmail} ${client.companyName || ""}`.toLowerCase();
  ...
  const hasNextPage =
    (pagination.pageIndex + 1) * pagination.pageSize < allCompanies.length;
                ...
                companies.map((company) => (
                  ...
                  >
                    <TableCell className="uppercase">
                      {highlightMatch(company.companyName, combinedFilter)}
                    </TableCell>
                    <TableCell>
                      {highlightMatch(
...
const CompanySelectButton: React.FC<CompanySelectButtonProps> = ({
  onCompanySelect,
}) => {
